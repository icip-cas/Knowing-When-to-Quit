...
def agg_loss(
    ...
    if loss_agg_mode == "token-mean":
        ...
        loss = verl_F.masked_sum(loss_mat, loss_mask) / (batch_num_tokens * dp_size).clamp(min=1)
        ...
        if loss_agg_mode.startswith("seq-mean-token-sum"):
            ...
            token_counts = torch.sum(loss_mask, dim=-1).clamp(min=1)  # per-sequence token count
            # token-mean per sequence
            seq_losses = verl_F.masked_sum(loss_mat, loss_mask, axis=-1) / token_counts
        else:
            raise ValueError(f"Invalid {loss_agg_mode=}")
